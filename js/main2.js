    ...
    if (searchBar.value != "") {
      (async function () {
        const req = await fetch(
          `https://api.weatherapi.com/v1/search.json?key=61f6dc3114064d2f9a8115945240201&id=2657545&q=${searchBar.value}`
        );
        if (req.status == 200) {
          ...
          for (let i = 0; i < tableBody.children.length; i++) {
            document
              .querySelector("tbody")
              .rows[i].cells[0].addEventListener("click", function (e) {
...
function getDegree(whichCity) {
  ...
  req.open(
    "get",
    `https://api.weatherapi.com/v1/forecast.json?key=bcfa934bdf84454e9d4214205240701&q=${whichCity}&days=3`
  );
  req.send();
  req.addEventListener("loadend", function () {
    if (req.status >= 200 && req.status <= 300) {
      ...
      nameOfCity.innerHTML =
        finalResult.location.name + ", " + finalResult.location.region;
      currentIconCondition.setAttribute(
        "src",
        finalResult.current.condition.icon
      );
      ...
      tomorrowIcon.setAttribute(
        "src",
        finalResult.forecast.forecastday[1].day.condition.icon
      );
      tomorrowText.innerHTML =
        finalResult.forecast.forecastday[1].day.condition.text;
      tomorrowMaxDeg.innerHTML =
        finalResult.forecast.forecastday[1].day.maxtemp_c + "&deg;C";
      tomorrowMinDeg.innerHTML =
        finalResult.forecast.forecastday[1].day.mintemp_c + "&deg;C";

      overmorrowIcon.setAttribute(
        "src",
        finalResult.forecast.forecastday[2].day.condition.icon
      );
      overmorrowText.innerHTML =
        finalResult.forecast.forecastday[2].day.condition.text;
      ...
      if (Number(finalResult.current.temp_c) >= 24) {
        ...
        Number(finalResult.current.temp_c) < 24 &&
        Number(finalResult.current.temp_c) > 5
      ) {
